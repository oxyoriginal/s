...
def add_item(request):
    ...
    if request.method == 'POST':
        ...
        service = Service.objects.create(text=request.POST['text'], title=request.POST['title'],category=category, user=request.user)
        service.save()
        msg = 'ok'
...
def update_item(request, id):
    ...
    if request.method == 'POST':
        category = Category.objects.get(id=request.POST['category'])
        Service.objects.filter(id=id).update(text=request.POST['text'], title=request.POST['title'],
                                             category=category)
